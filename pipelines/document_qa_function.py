...
def _query_webui_collections(
    ...
    print(f"[DocQA] Semantic query returned {len(all_chunks)} chunks")
        ...
        for eq in extra_queries:
            ...
            for text, meta in extra:
                if text not in seen_texts:
                    ...
                    added += 1
            if added:
                print(f"[DocQA] Keyword query '{eq}' added {added} new chunks")
...
class Pipe:
    ...
    def pipe(
        ...
        if date_hint:
            print(f"[DocQA] Date detected in query: {date_hint[0]} {date_hint[1]}")
